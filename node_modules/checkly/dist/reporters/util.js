...
import { formatAssertionLine, truncate, } from '../formatters/assertion-line.js';
...
function formatTracerouteResponse(response) {
    ...
    const hopLines = hops.slice(0, 40).map((hop) => {
...
function formatGrpcResponse(response) {
    ...
    const metadataLines = metadata.slice(0, 20).map((m) => `  ${m.key ?? m.name ?? '(key)'}: ${m.value ?? ''}`);
...
function formatSslResponse(response, limits) {
...
export function formatRunError(err) {
