...
import { CheckSessionWaitTimeoutError, NoMatchingChecksError, } from '../../rest/check-sessions.js';
...
export default class ChecksRun extends AuthCommand {
    ...
    async run() {
        ...
        try {
            ...
            const format = flags.output === 'md' ? 'md' : 'terminal';
            ...
            if (err instanceof CheckSessionWaitTimeoutError) {
                this.style.longError('Timed out waiting for check sessions.', `The check sessions are still running in Checkly. Increase --timeout or use --detach to exit after scheduling.`);
                process.exitCode = 1;
                return;
            }
