...
export class BaseCommand extends Command {
    ...
    async checkSkillFreshness() {
        ...
        try {
            ...
            const entries = stale
                .map(({ dir, targetPath }) => `${relative(process.cwd(), targetPath)}\n`
                + `  npx checkly skills install --path ${dir} --force`)
                .join('\n\n');
            this.style.longWarning('Checkly skill is out of date', `The installed skill no longer matches this CLI version. `
    ...
    async init() {
        ...
        if (version === '0.0.1-dev' || version?.startsWith('0.0.0')) {
            try {
                ...
                const { data: packageInformation } = await axios.get(registryUrl, assignProxy(registryUrl, {}));
