...
import { Check, AlertChannelSubscription, AlertChannel, CheckGroup, Dashboard, MaintenanceWindow, PrivateLocation, PrivateLocationCheckAssignment, PrivateLocationGroupAssignment, Diagnostics, Session, StatusPage, StatusPageService, } from '../constructs/index.js';
...
(function (ResourceDeployStatus) {
    ...
    ResourceDeployStatus["DETACHED"] = "DETACHED";
})(ResourceDeployStatus || (ResourceDeployStatus = {}));
const PRETTY_RESOURCE_TYPES = {
...
export default class Deploy extends AuthCommand {
    ...
    async run() {
        ...
        const { force, preview, 'cancel-in-progress-deployment': cancelInProgress, 'schedule-on-deploy': scheduleOnDeploy, 'preserve-resources': preserveResources, output: outputFlag, verbose, config: configFilename, 'verify-runtime-dependencies': verifyRuntimeDependencies, 'debug-bundle': debugBundle, 'debug-bundle-output-file': debugBundleOutputFile, } = flags;
        ...
        if (!preview && !preserveResources && !force) {
            ...
            try {
                const { data: dryRunData } = await api.projects.deploy({ ...projectPayload, repoInfo }, { dryRun: true, scheduleOnDeploy, preserveResources });
            ...
            if (deletions.length) {
                ...
                await this.confirmOrAbort({
                    ...
                    changes: [
                        ...
                        ...deletions.map(({ resourceType, logicalId }) => `Delete ${PRETTY_RESOURCE_TYPES[resourceType] ?? resourceType}: ${logicalId}`),
                    ],
                    flags,
                    flagMetadata: metadata.flags,
                    classification: {
        ...
        try {
            ...
            const { data } = await api.projects.deploy({ ...projectPayload, repoInfo }, {
                dryRun: preview,
                scheduleOnDeploy,
                preserveResources,
                cancelInProgress,
            ...
            if (err instanceof ProjectDeployCancelledError) {
                ...
                this.style.longError('A deployment for this project is still in progress.', 'Try again later, or re-run with `--cancel-in-progress-deployment` to '
    ...
    collectDeletions(previewData) {
        return (previewData?.diff ?? [])
            .filter(change => change.action === ResourceDeployStatus.DELETE
            && !NON_REPORTED_TYPES.some(t => t === change.type))
            .map(({ type, logicalId }) => ({ resourceType: type, logicalId }))
            .sort((a, b) => a.resourceType.localeCompare(b.resourceType) || a.logicalId.localeCompare(b.logicalId));
    }
