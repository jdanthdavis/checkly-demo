...
class Projects {
     ...
     */
    async deleteProject(logicalId, { preserveResources = false, cancelInProgress = false, onProgress, onStatus } = {}) {
     ...
     */
    async submitDeletion(logicalId, { preserveResources, onProgress }) {
        const { data } = await this.api.delete(`/v1/projects/${encodeURIComponent(logicalId)}`, { params: { preserveResources, dryRun: false } });
        ...
        if (completed.status === 'CANCELLED') {
            throw new ProjectDeployCancelledError('The deletion was cancelled before it finished. A newer operation may have superseded it.');
        }
     ...
     */
    async deploy(resources, { dryRun = false, scheduleOnDeploy = true, preserveResources = false, cancelInProgress = false, onProgress, onStatus, } = {}) {
    ...
    async submitDeployment(resources, { dryRun, scheduleOnDeploy, preserveResources, onProgress }) {
        ...
        const { data } = await this.api.post(`/v1/projects/deploy?dryRun=${dryRun}&scheduleOnDeploy=${scheduleOnDeploy}${preserveParam}`, resources, { transformRequest: compressJSONPayload });
        ...
        if (completed.status === 'CANCELLED') {
            throw new ProjectDeployCancelledError('A newer deployment may have cancelled yours. Try deploying again if you still need to apply your changes.');
        }
     ...
     */
    async resolveInProgressDeployment(logicalId, deploymentId, { cancel, onStatus, deadlineAt }) {
        if (cancel) {
    ...
    getDeployment(logicalId, deploymentId) {
        return this.api.get(`/v1/projects/${encodeURIComponent(logicalId)}/deployments/${encodeURIComponent(deploymentId)}`);
    }
    /** Request cancellation of an in-flight deployment (idempotent on the server). */
    cancelDeployment(logicalId, deploymentId) {
        return this.api.post(`/v1/projects/${encodeURIComponent(logicalId)}/deployments/${encodeURIComponent(deploymentId)}/cancel`);
    }
    /**
     ...
     */
    async awaitDeploymentCompletion(logicalId, deploymentId, { maxWaitSeconds = 30 } = {}) {
        const { data } = await this.api.get(`/v1/projects/${encodeURIComponent(logicalId)}/deployments/${encodeURIComponent(deploymentId)}/completion`, { params: { maxWaitSeconds } });
        return data;
    }
    /**
     ...
     * no cursor.
     */
    async streamDeploymentEvents(logicalId, deploymentId, { onProgress, maxReconnects = 5 } = {}) {
        let reconnects = 0;
        for (;;) {
            try {
    ...
    async openEventStream(logicalId, deploymentId) {
        try {
            const { data } = await this.api.get(`/v1/projects/${encodeURIComponent(logicalId)}/deployments/${encodeURIComponent(deploymentId)}/events`, { responseType: 'stream', headers: { Accept: 'text/event-stream' } });
            return data;
        }
        catch (err) {
    ...
    async consumeEventStream(logicalId, deploymentId, onProgress) {
