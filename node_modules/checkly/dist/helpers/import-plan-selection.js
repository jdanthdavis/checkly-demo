 ...
 */
function planFlagsConflict({ all, planId }) {
 ...
 * exist remotely.
 */
export function validatePlanFlagsOrExit(command, flags) {
 ...
 * error.
 */
export function reportNoCandidatePlans(command, plans, options) {
    if (plans.length > 0) {
        return false;
    }
 ...
 */
export function resolvePlanNonInteractively(plans, planId, action) {
    if (planId !== undefined) {
        const plan = plans.find(candidate => candidate.id === planId);
        if (plan === undefined) {
            throw new PlanSelectionError(`No plan available to ${action} with ID "${planId}".`);
        }
        return plan;
    }
    ...
    throw new PlanSelectionError(`Found ${plans.length} plans available to ${action}. `
 ...
 */
export async function selectPlanOrExit(command, plans, planId, action, interactiveFallback) {
    try {
 ...
 */
export async function selectPlansOrExit(command, plans, { all, planId }, action, interactiveFallback) {
    try {
...
//# sourceMappingURL=import-plan-selection.js.map