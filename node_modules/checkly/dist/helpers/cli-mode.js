...
const DISABLED_AGENT_VALUES = new Set(['0', 'false', 'no', 'off']);
const GENERIC_AGENT_VALUES = new Set(['1', 'true', 'yes', 'on']);
const RESERVED_OPERATOR_NAMES = new Set(['manual']);
const VALID_OPERATOR_NAME = /^[a-z0-9][a-z0-9_-]{0,63}$/;
function normalizeExplicitAgent(value) {
    const normalized = value?.trim().toLowerCase();
    if (!normalized || DISABLED_AGENT_VALUES.has(normalized))
        return undefined;
    if (GENERIC_AGENT_VALUES.has(normalized))
        return 'agent';
    if (RESERVED_OPERATOR_NAMES.has(normalized) || !VALID_OPERATOR_NAME.test(normalized))
        return 'agent';
    return EXPLICIT_AGENT_ALIASES[normalized] ?? normalized.replaceAll('_', '-');
}
function detectExplicitAgent() {
    return normalizeExplicitAgent(process.env.AI_AGENT)
        ?? normalizeExplicitAgent(process.env.AGENT);
}
export function detectOperator(fileExists = existsSync) {
    ...
    if (explicitAgent && explicitAgent !== 'agent')
    ...
    if (process.env.AMP_CURRENT_THREAD_ID)
        return 'amp';
    ...
    if (process.env.CURSOR_TRACE_ID
        ...
        return 'cursor';
    if (process.env.VSCODE_AGENT || process.env.COPILOT_AGENT)
        return 'vscode-agent';
    ...
    if (process.env.AIDER)
        return 'aider';
    if (process.env.WINDSURF || process.env.WINDSURF_AGENT || process.env.CODEIUM_ENV)
        return 'windsurf';
    ...
    if (process.env.ANDROID_STUDIO_AGENT)
        return 'android-studio-agent';
    if (process.env.KIRO_AGENT_PATH)
        return 'kiro-agent';
    if (process.env.TRAE_AI_SHELL_ID)
        return 'trae';
    if (process.env.GOOSE_TERMINAL)
        return 'goose';
    if (process.env.GITHUB_AW === 'true')
        return 'github-agentic-workflow';
    if (fileExists('/opt/.devin'))
        return 'devin';
    if (process.env.HOME === '/home/jules' && fileExists('/opt/environment_summary.sh'))
        return 'jules';
    if (process.env.PATH?.includes('/.pi/agent') || process.env.PATH?.includes('\\.pi\\agent'))
        return 'pi';
    // Boolean generic markers are deliberately checked after specific signals such as OPENCODE=1.
    if (explicitAgent === 'agent')
        return 'agent';
    ...
    if (process.env.MONOSPACE_ENV || fileExists('/google/idx'))
...
export function detectCliMode(fileExists = existsSync) {
