...
export class PlaywrightConfigExpander {
    ...
    async #findTestFiles(playwrightConfig, options) {
     ...
     */
    async #reconcile(files, playwrightConfig, options) {
        ...
        const inRoot = (file) => file === bundleRoot || file.startsWith(bundleRoot + path.sep)
        ...
        const spelledFallbacks = Array.from(playwrightConfig.referencedPaths)
            .filter(([spelled, canonical]) => spelled !== canonical && inRoot(spelled) && !inRoot(canonical))
        ...
        return files.map(file => {
            ...
            throw new Error(`${file} is outside the project's bundle root (${bundleRoot}) and cannot be included `
