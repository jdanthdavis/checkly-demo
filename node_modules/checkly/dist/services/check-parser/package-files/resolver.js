 ...
 */
function configFileCandidates(base) {
...
class PackageFilesCache {
    ...
    #tsconfigSchemaCache = new FileLoader(async (filePath) => {
        const sourceFile = await this.#sourceFileCache.load(filePath);
        if (sourceFile === undefined) {
            return;
        }
        return JsonTextSourceFile.loadFromSourceFile(sourceFile);
    });
    ...
    #resolveExtends = async (specifier, fromDir) => {
    ...
    async #loadExtendedConfig(candidates, bundle) {
    ...
    #tsconfigJsonCache = new FileLoader(this.#jsonTextFileLoader((jsonFile) => TSConfigFile.loadFromJsonTextSourceFile(jsonFile, this.#resolveExtends)));
    #jsconfigJsonCache = new FileLoader(this.#jsonFileLoader((jsonFile) => JSConfigFile.loadFromJsonSourceFile(jsonFile, this.#resolveExtends)));
...
export class PackageFilesResolver {
    ...
    async resolveDependenciesForFilePath(filePath, dependencies) {
        ...
        const pushExtendedConfigs = (importPath, configFile) => {
        ...
        if (this.workspace && !this.restricted) {
            ...
            for (const pkg of [this.workspace.root, ...this.workspace.packages]) {
                const npmrc = await this.cache.exactSourceFile(path.join(pkg.path, NPMRC_FILENAME));
                if (npmrc !== undefined) {
        ...
        const resolveWorkspaceOrExternal = async (specifier, source) => {
            if (this.workspace) {
