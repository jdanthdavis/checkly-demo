...
export async function loadWorkspaceCacheHashInputs(workspace) {
    ...
    const npmrcResults = await Promise.all(allPackages.map(async (pkg) => {
    ...
    const npmrcs = npmrcResults.filter((entry) => entry !== undefined);
    let lockfile;
 ...
 */
export function normalizeDependencyCacheVersion(version) {
 ...
 */
export async function computeWorkspaceCacheHash(workspace, options) {
