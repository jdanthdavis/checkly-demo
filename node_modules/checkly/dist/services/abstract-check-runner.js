...
function schedulingFailedError(operation) {
    return new TestSessionSchedulingFailedError(operation.error?.message ?? 'The test session could not be scheduled.', { code: operation.error?.code });
}
export default class AbstractCheckRunner extends EventEmitter {
     ...
     */
    watchScheduling(schedulingId, signal) {
     ...
     */
    async assertSchedulingSucceeded(schedulingId) {
        if (!schedulingId) {
            return;
        }
        let operation;
